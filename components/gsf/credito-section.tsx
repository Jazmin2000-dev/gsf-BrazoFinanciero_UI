...
export function CreditoSection() {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, margin: "-100px" });

  const capabilities = [
    { icon: TermsIcon, label: "Organizar el financiamiento a clientes" },
    { icon: InterCompanyIcon, label: "Estructurar crédito inter-compañía" },
    { icon: CashFlowIcon, label: "Gestionar el flujo de efectivo" },
    { icon: LeasingIcon, label: "Arrendar equipo y maquinaria" },
    { icon: IncomeIcon, label: "Generar ingresos financieros adicionales" },
  ];
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          >
            ...
            <h2 className="font-heading text-3xl md:text-5xl font-bold text-foreground mb-5 text-balance">
              El crédito es una oportunidad de negocio, no un favor
            </h2>
            <p className="text-lg text-muted-foreground leading-relaxed mb-6 text-pretty">
              Cuando dejas el financiamiento en manos de una entidad externa, regalas el margen
              financiero de tus propias ventas. Un brazo financiero te permite capturar ese margen y
              convertirlo en una nueva línea de ingresos.
            </p>
            <div className="rounded-2xl border border-primary/20 bg-primary/5 p-6">
              <p className="text-base text-foreground leading-relaxed">
                <span className="font-semibold text-primary">El cambio de mentalidad:</span> deja de
                ver el crédito como un riesgo a evitar y empieza a verlo como un producto rentable que
                fortalece tus ventas.
              </p>
            </div>
          </motion.div>
